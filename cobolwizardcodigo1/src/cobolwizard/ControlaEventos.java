...
public class ControlaEventos implements ActionListener{
	...
	public void actionPerformed(ActionEvent evento)
    {
		...
		if (evento.getSource()==ventana.jbNext){//si hay evento en el boton examinar
			...
			if(cadena.length()>0 && root.length()>0 && copy.length()>0)
		       {
		        
				if(cadena.matches("[a-zA-Z0-9]*")){
					if(dir.exists() && dir1.exists()){
						if(!ventana.jchkmemoria.isSelected()){
							generar = new Generar(ventana.jtfproyecto.getText(),ventana.jtfRutaEntrada.getText(),ventana.jtfRutaEntradaC.getText(),ventana.noextension,ventana.debug,ventana.jchkdebug.isSelected());
							ventana.dispose();
						}
						else{
							generar = new Generar(ventana.jtfproyecto.getText(),ventana.jtfRutaEntrada.getText(),ventana.jtfRutaEntradaC.getText(),ventana.noextension,ventana.debug,ventana.jchkdebug.isSelected(),memory);
							ventana.dispose();
						}
					}else{
