...
print "validos: ".$numero_de_archivos_validos."\n";
...
#Scan:
  ...
  $comando = 'sourceanalyzer -64 -Xmx'. $memory . ' -Xss'. $memoryxss .
      ' -b ' . $project . ' -scan -f ' . $route ."\\BXI_BEM_B\\results.fpr";
  system($comando) == 0 or die "El Escaneo". $project ." ha fallado";

  #Geracion de archivos xml y pdf
  print 'Generando Reporte PDF\n';
  $comando = 'ReportGenerator -format xml -template harvest.xml -f ' . $route ."\\BXI_BEM_B\\results.xml -source ". $route ."\\BXI_BEM_B\\results.fpr";
  system($comando) == 0 or die "No se pudo crear el archivo xml";
  $comando = 'ReportGenerator -format pdf -template harvest.xml -f ' . $route ."\\BXI_BEM_B\\results.pdf -source ". $route ."\\BXI_BEM_B\\results.fpr";
  ...
  $comando = 'fortifyclient -url "' . $url . '" -authtoken "' . $token. 
      '" uploadFPR -file "' . $route .'\\BXI_BEM_B\\results.fpr" -project "' . $project . '" -version "' . $parcial . '"';
  ...
  $comando="java -jar C:\\Harvest-FF\\lib\\Validador_de_resultados_Beta.jar " . $route ."\\BXI_BEM_B ". $completo.' '.$parcial.' '.$package .' '.$email ;
  system($comando) == 0 or die  "No se pudo realizar la verificacion revisar verificar.log";
  
  #Respaldos de FPRS y borrado de carpetas  
  $comando="Copy \"". $route ."\\BXI_BEM_B\\results.fpr\" \"C:\\Harvest-FF\\FPRS\\".$project.$fecha.".fpr\"";
